...
func_app_prereq() {
  ...
  #add user roboshop - check if user exist or no if no then add user - if error handle it by exit
...
func_schema_setup() {
  if [ "$schema_setup" == "mongo" ]; then
    ...
    mongo --host mongodb-dev.gangabhavanikatraparthi.online </app/schema/${component}.js &>>$log_file
    func_stat_check $?
  fi
